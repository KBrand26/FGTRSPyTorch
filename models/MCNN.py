...
class MCNN(pl.LightningModule):
    """
    Merged Convolution Neural Network that has an additional region for extracting certain features that are fed back into the rest
    of the network, which is responsible for classification.
    """
    def __init__(self, run, filters=64, nconv=2, lr=0.0001, main_weight=0.5, aux_weight=0.5):
        ...
        Args:
            ...
            main_weight (float, optional): The weight used for the classification loss.
            aux_weight (float, optional): The weight used for the feature extraction loss.
        """
        super().__init__()
        ...
        conv_layers.append(nn.Dropout1d(0.5))
        self.conv_base = nn.Sequential(*conv_layers)
        
        # Free neurons
        unguided_layers = []
        unguided_layers.append(
            nn.Linear(
                in_features=150,
                out_features=71
            )
        )
        unguided_layers.append(nn.ELU())
        self.free_layer = nn.Sequential(*unguided_layers)
        ...
        output_layers = []
        output_layers.append(nn.Dropout1d(0.5))
        ...
        self.class_out = nn.Sequential(*output_layers)
        
        # Initialize weights
        self.conv_base.apply(initialize_weights)
        self.free_layer.apply(initialize_weights)
        self.class_out.apply(initialize_weights)
    ...
    def forward(self, x):
        ...
        # Pass through free neurons
        free_x = self.free_layer(x)
        # Pass through guided neurons
        bent_x = self.bent_out(x)
        fr_x = self.fr_out(x)
        core_count_x = self.core_count_out(x)
        core_ratio_x = self.core_ratio_out(x)
        
        # Concatenate outputs
        feature_outs = torch.cat((free_x, bent_x, fr_x, core_count_x, core_ratio_x), 1)
        
        # Pass through output layer
        class_out = self.class_out(feature_outs)
        return feature_outs, class_out, bent_x, fr_x, core_count_x, core_ratio_x
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        self.log('test_comp _prec', prec[1])
